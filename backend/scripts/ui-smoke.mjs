/* UI smoke-тест админки «как пользователь».
   Заходит в прод-админку с ADMIN_TOKEN, реально кликает ключевые функции
   и проверяет что они открылись (а не просто «код на месте»).
   Запуск:
...
async function run() {
  ...
  // вход через токен (как реальный admin-сеанс)
  ...
  await T('Логин прошёл (экран входа исчез)', async () => {
    const vis = await p.locator('.login-card').isVisible().catch(() => false);
    if (vis) throw new Error('login-card всё ещё видна');
  });

  await T('Дашборд: основные функции определены в JS', async () => {
    const d = await p.evaluate(() => ({ e: typeof openEmbed, dr: typeof openDrill, g: typeof go }));
    if (d.e !== 'function' || d.dr !== 'function' || d.g !== 'function')
      throw new Error('функции не определены: ' + JSON.stringify(d));
  });
  ...
  await T('Дашборд → клик «Відтік клієнтів» открывает модалку розбору', async () => {
    ...
    const vis = await p.locator('#drillModal').isVisible();
    if (!vis) throw new Error('drillModal не открылась');
    await p.locator('#drillModal [onclick*="closeDrill"]').first().click({ timeout: 4000 }).catch(() => {});
  });

  await T('План роботи адміністратора виден на дашборде', async () => {
    const vis = await p.locator('#planCard').isVisible().catch(() => false);
    if (!vis) throw new Error('planCard скрыта');
  });

  await T('Маркетинг → AI Відеостудія открывается (iframe грузится)', async () => {
    // раскрыть свёрнутую группу «Маркетинг», если закрыта
    ...
    if (await header.count()) {
      ...
      if (!open) { await header.click({ timeout: 5000 }); await p.waitForTimeout(600); }
    }
    ...
    if (!ifr) throw new Error('iframe видеостудии не появился');
  });
