...
router.get('/overview', requirePerm(), cacheReport(), async (req, res) => {
  try {
    ...
    const [revToday, revMonth, expMonth, apptToday, newClientsMonth, topMasters, openShift] = await Promise.all([
      ...
      // расходы месяц: ПОВНІ витрати + окремо виплати майстрам (рішення Боса 28.06, варіант B)
      pool.query(`SELECT
    ...
    if (canFinance) {
      ...
      out.expense_month = Number(expMonth.rows[0].total);           // ПОВНІ витрати (вкл. ЗП майстрів)
      out.master_payouts_month = Number(expMonth.rows[0].masters);  // виплати майстрам — окремо
      out.opex_month = out.expense_month - out.master_payouts_month;            // витрати без ЗП майстрів
      out.profit_month = out.revenue_month.total - out.opex_month;             // прибуток ДО виплат майстрам
      out.net_profit_month = out.revenue_month.total - out.expense_month;       // чистий прибуток (після майстрів)
