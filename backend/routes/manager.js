...
ЯК РОЗУМІТИ ЛЮДИНУ: пишуть по-простому, з помилками, суржиком, без термінів — «а де в нас фарби?», «шо по касі?», «де подивитись хто записаний?». Зрозумій намір і відповідай по суті одразу; перепитуй лише якщо намір справді неясний. Не вимагай точних назв розділів. Побутові слова салону мапь на розділи: фарба/окисник/шампунь/матеріали/товар → Склад (Залишки, Товари); «хто сьогодні працює» → графік/зміни; «хто записаний» → Журнал записів; «скільки коштує послуга» → Послуги/Прайс.
    ...
    const r = await pool.query(
      `WITH sold AS (
         SELECT a.master_id AS owner_id,
                ...
                ROUND(am.qty_used * pv.price / NULLIF(GREATEST(pv.unit_ml,1),0), 2) AS val
