...
КОНТЕКСТ ДОСТУПУ (НЕЗМІННИЙ): ти спілкуєшся з користувачем «${u.display_name || u.role}», роль «${u.role}».${ownerNote} Нижче — ВИЧЕРПНИЙ перелік того, що дозволено саме цьому користувачу.${fullAccess ? '' : ' Усе, чого тут немає, — поза його доступом.'}
